...
import arithmetic_v2 as arithmetic
import time
...
class EncWrapper(nn.Module):
    ...
    def forward(self, x, jpg_patch):
        ...
        samples = samples.reshape(B, 1, 1, -1).repeat(1, 1, mix_num, 1)
        valid_pos = valid_pos.reshape(B, 1, 1, -1).repeat(1, 1, mix_num, 1)
        ...
        for i in range(max_step):
            ...
            mu, log_sigma, coeffs, weights = torch.split(ep_params, 4 * mix_num, dim=1)
            if self.codec.distribution.no_multichannel_lmm:
                weights = weights.reshape(B, 1, mix_num, -1, 1)
                weights = weights.repeat(1, 4, 1, 1, 1)
            else:
                weights = weights.reshape(B, 4, mix_num, -1, 1)
            ...
            for c in range(4):
                if c == 0:
                    ...
                    mu_c = (
                        mu[:, mix_num2:mix_num3, :, :]
                        + (x_crop[:, 1:2, :] * norm_scale) * coeffs[:, mix_num:mix_num2, :, :]
                    )
                    mu_c = mu_c.permute(0, 2, 1, 3)
                else:
                    mu_c = (
                        mu[:, mix_num3:, :, :]
                        + (x_crop[:, 0:1, :] * norm_scale) * coeffs[:, mix_num2:mix_num3, :, :]
                        + ((x_crop[:, 1:2, :] + x_crop[:, 2:3, :]) / 2 * norm_scale) * coeffs[:, mix_num3:, :, :]
                    )
                    # mu_c = (
                    #     mu[:, mix_num3:, :, :]
                    #     + (x_crop[:, 0:1, :] * norm_scale) * coeffs[:, mix_num2:mix_num3, :, :]
                ...
                pmf = pmf.clamp_(0, 1.0)
                pmf = pmf / torch.sum(pmf, dim=2, keepdim=True)
                cdf = torch.cumsum(pmf, dim=2).clamp_(0.0, 1.0)
                cdf = F.pad(cdf, (1, 0))
                symbol = x_crop[:, c].short().reshape(B, -1)
                # print(f"{i} idx Encoding cdf shape:", cdf.shape, "symbol shape:", symbol.shape)
                # time1 = time.time()
                cdf = cdf.view(-1, max_width + 1)
                symbol = symbol.view(-1)
                stream, cnt = arithmetic.arithmetic_encode(
                    symbol, cdf, self.chunk_sz, int(cdf.shape[0]), int(max_width + 1)
                )
                # time2 = time.time()
                # print(f"Encoding time : {time2 - time1} seconds")
                x_stream.append((stream, cnt))
            # t2 = time.time()
...
class DecWrapper(nn.Module):
    ...
    def forward(self, latent_code, x_stream, bit_depth, jpg_patch):
        ...
        x_tmp = torch.zeros(prior_total.shape[0], 4, prior_total.shape[2], prior_total.shape[3], device=device)
        COT = self.codec.sp_ctx.get_coding_table()
        max_step = torch.max(COT)
        ...
        for i in range(max_step):
            ...
            if self.codec.distribution.no_multichannel_lmm:
                ...
                weights = weights.reshape(weights.shape[0], 4, mix_num, -1, 1)
            coeffs = torch.tanh(coeffs)
            for c in range(4):
                if c == 0:
                    mu_c = mu[:, :mix_num, :, :].permute(0, 2, 1, 3)
                ...
                symbol_out = arithmetic.arithmetic_decode(
                    cdf, x_stream[j][0], x_stream[j][1], self.chunk_sz, int(cdf.shape[0]), int(max_width + 1)
                )
