...
class PriorCompressor(CompressionModel):
    ...
    def forward(self, x):
        ...
        z = self.h_a(y)  # TODO downsample n times
    ...
    def compress(self, x):
        ...
        z_strings = self.z_dist.compress(z)

        z_hat = self.z_dist.decompress(z_strings, z.size()[-2:], z.size()[:2])
    ...
    def decompress(self, strings, shape):
        ...
        z_hat = self.z_dist.decompress(strings[1], shape[2:], shape[:2])
        mu_hat, sigma_hat = self.h_s(z_hat)
